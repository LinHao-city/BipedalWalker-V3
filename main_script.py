...
try:
    ...
    Monitor = None
    print("Warning: RecordVideo not available, video recording disabled")
import torch
import numpy as np
...
from src.utils.env_wrappers import BoxToHistoryBox, MyWalkerWrapper
import argparse
import os
...
if args.model_type=='ff':
    from src.architectures.feedforward import Actor, Critic
elif args.model_type=='trsf':
    from src.architectures.transformer import Actor, Critic
elif args.model_type=='mlp':
    from src.architectures.mlp import Actor, Critic
elif args.model_type=='rnn':
    # 注意：原项目中没有rnn_models.py文件
    print('RNN model not available in current structure, using feedforward instead')
    from src.architectures.feedforward import Actor, Critic
elif args.model_type=='lstm':
    from src.architectures.lstm import Actor, Critic
elif args.model_type=='bilstm':
    from src.architectures.bilstm import Actor, Critic
else:
...
if args.flag == 'train':
    ...
    agent.eval_mode()
    #env.seed(0)
    env = Monitor(env, os.path.join('.', 'evaluation_results', 'videos'))
