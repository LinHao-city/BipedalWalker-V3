...
try:
    ...
    # 添加archs/utils到路径
    archs_utils_path = str(Path(__file__).parent.parent.parent / "archs" / "utils")
    if archs_utils_path not in sys.path:
        sys.path.append(archs_utils_path)
    try:
        from transformer import PositionalEncoding, LearnablePositionalEncoding, TransformerLayer
    except ImportError as e:
        # 最后的备用方案：内联定义或抛出更清晰的错误
        raise ImportError(
            f"Cannot import transformer components. "
            f"Please ensure that transformer.py and mha.py are available in archs/utils/. "
            f"Original error: {e}"
        )
